...
<head>
    ...
    <script type="module">
        ...
        async function checkAndSetAuthorityLevel(userId) {
            ...
            if (!userData || userData.authorityLevel === undefined) {
                // If user data doesn't exist or authorityLevel is not set, set it to 1
                await set(userRef, { 
                    ...userData, 
                    authorityLevel: 1 
                });
            ...
            return userData.authorityLevel;
        }
        ...
        async function emailPasswordSignIn(email, password) {
            try {
                ...
                console.log(`User ${userCredential.user.uid} has authority level: ${authorityLevel}`);
        ...
        async function registerUser(email, password) {
            ...
            try {
                ...
                await checkAndSetAuthorityLevel(userCredential.user.uid);
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            updateAuthorityLevelInHeader();
        });

        // Function to update the authority level in the header
        function updateAuthorityLevelInHeader() {
            ...
            if (currentUser) {
                ...
                userRef.on('value', snapshot => {
                    const userData = snapshot.val();
                    const authorityLevel = userData ? userData.authorityLevel : 0;
                    const authorityElement = document.getElementById('authorityLevel');
                    if (authorityElement) {
                        authorityElement.textContent = `Authority Level: ${authorityLevel}`;
                    }
                });
            }
        }
